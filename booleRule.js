...
function findIntegral(coeffs, interval, dec) {
    if (document.getElementById("eqn").value !== "") {
        let h = (interval[1] - interval[0]) / 4
        let n = (interval[1] - interval[0]) * (((7 * getY(coeffs, interval[0])) + (32 * getY(coeffs, interval[0] + h)) + (12 * getY(coeffs, interval[0] + 2*h)) + (32 * getY(coeffs, interval[1] - h)) + (7 * getY(coeffs, interval[1]))) / 90)
        n = n.toFixed(dec)
        return (n)
    }
    else {
