        ...
        if (getY(coeffs, interval[0]) * getY(coeffs, interval[1]) < 0) {
            tableStr += `
                <tr>
                    ...
                    <td>${getXr(interval[0],interval[1],getY(coeffs, interval[0]),getY(coeffs, interval[1])).toFixed(dec)}</td>
                    <td>${getY(coeffs, getXr(interval[0],interval[1],getY(coeffs, interval[0]),getY(coeffs, interval[1]))).toFixed(dec)}</td>
                    <td>${getDiff(getXr(interval[0],interval[1],getY(coeffs, interval[0]),getY(coeffs, interval[1])), interval[interval[2]]) + "%"}</td>
                </tr>`
            s++
            if (getDiff(getXr(interval[0],interval[1],getY(coeffs, interval[0]),getY(coeffs, interval[1])), interval[interval[2]]) <= err ||
                itr <= 0) {
                console.log("err: " + err + " error: " + getDiff(getcenter(interval[0], interval[1]), interval[interval[2]]))
                return getXr(interval[0],interval[1],getY(coeffs, interval[0]),getY(coeffs, interval[1])).toFixed(dec)
            }
            else {



                let center = getXr(interval[0],interval[1],getY(coeffs, interval[0]),getY(coeffs, interval[1]))
